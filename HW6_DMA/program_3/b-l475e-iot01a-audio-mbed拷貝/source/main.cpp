...
static DigitalOut pin1(D8);
static DigitalOut pin2(D9);
...
void target_audio_buffer_full() {
    ...
    uint8_t wav_header[44] = {
        ...
        static_cast<std::uint8_t>(fileSize & 0xff), static_cast<std::uint8_t>((fileSize >> 8) & 0xff), static_cast<std::uint8_t>((fileSize >> 16) & 0xff), static_cast<std::uint8_t>((fileSize >> 24) & 0xff),
        0x57, 0x41, 0x56, 0x45, // WAVE
        0x66, 0x6d, 0x74, 0x20, // fmt
        0x10, 0x00, 0x00, 0x00, // length of format data
        0x01, 0x00, // type of format (1=PCM)
        0x01, 0x00, // number of channels
        static_cast<std::uint8_t>(wavFreq & 0xff), static_cast<std::uint8_t>((wavFreq >> 8) & 0xff), static_cast<std::uint8_t>((wavFreq >> 16) & 0xff), static_cast<std::uint8_t>((wavFreq >> 24) & 0xff),
        0x00, 0x7d, 0x00, 0x00, // 	(Sample Rate * BitsPerSample * Channels) / 8
        0x02, 0x00, 0x10, 0x00,
        0x64, 0x61, 0x74, 0x61, // data
        static_cast<std::uint8_t>(dataSize & 0xff), static_cast<std::uint8_t>((dataSize >> 8) & 0xff), static_cast<std::uint8_t>((dataSize >> 16) & 0xff), static_cast<std::uint8_t>((dataSize >> 24) & 0xff),
    };
