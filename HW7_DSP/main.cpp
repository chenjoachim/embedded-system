...
void PrintOut(int k){
    for(int i = k; i < k + SAMPLE_BUFFER_SIZE/2; i++){   
        printf("%d.", i);
        printf("%d\n", sample_buffer[i]);
    }
}
float32_t FIRinput[SAMPLE_BUFFER_SIZE/2];
void PrintFIR(){
...
{   
    //event_queue.call(PrintOut,SAMPLE_BUFFER_SIZE/2);
    for(int i = 0; i < SAMPLE_BUFFER_SIZE/2; i++){
        FIRinput[i] = (float32_t)sample_buffer[i + SAMPLE_BUFFER_SIZE/2];
    }
...
{   
    //event_queue.call(PrintOut,0);
    for(int i = 0; i < SAMPLE_BUFFER_SIZE/2; i++){
        FIRinput[i] = (float32_t)sample_buffer[i];
    }
    event_queue.call(do_FIR,FIRinput);
    event_queue.call(PrintFIR);
    return;
